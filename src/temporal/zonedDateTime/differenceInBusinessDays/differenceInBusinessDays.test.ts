...
describe("differenceInBusinessDays", () => {
  it("returns the number of business days between the given dates, excluding weekends", () => {
    const result = differenceInBusinessDays(newZDT("2014-07-18T00:00:00.000Z"), newZDT("2014-01-10T00:00:00.000Z"));
  ...
  it("can handle long ranges", () => {
    ...
    const result = differenceInBusinessDays(
      newZDT("2000-01-01T00:00:00.000Z").with({ year: 15000 }),
      newZDT("2014-01-01T00:00:00.000Z"),
    );
    expect(result).toBe(3387885);
  });

  it("the same except given first date falls on a weekend", () => {
    const result = differenceInBusinessDays(newZDT("2019-07-20T00:00:00.000Z"), newZDT("2019-07-18T00:00:00.000Z"));
    expect(result).toBe(2);
  });

  it("the same except given second date falls on a weekend", () => {
    const result = differenceInBusinessDays(newZDT("2019-07-23T00:00:00.000Z"), newZDT("2019-07-20T00:00:00.000Z"));
    expect(result).toBe(1);
  });

  it("the same except both given dates fall on a weekend", () => {
    const result = differenceInBusinessDays(newZDT("2019-07-28T00:00:00.000Z"), newZDT("2019-07-20T00:00:00.000Z"));
    expect(result).toBe(5);
  });

  it("returns a negative number if the time value of the first date is smaller", () => {
    const result = differenceInBusinessDays(newZDT("2014-01-10T00:00:00.000Z"), newZDT("2014-07-20T00:00:00.000Z"));
    expect(result).toBe(-135);
  });

  describe("businessDays option", function () {
    it("can include Saturday in businessDays", function () {
      const result = differenceInBusinessDays(newZDT("2022-01-17T00:00:00.000Z"), newZDT("2022-01-07T00:00:00.000Z"), {
    ...
    it("can include Saturday in businessDays given first date falls on a non-businessDay", function () {
      const result = differenceInBusinessDays(newZDT("2022-01-16T00:00:00.000Z"), newZDT("2022-01-07T00:00:00.000Z"), {
        businessDays: [1, 2, 3, 4, 5, 6],
      });
      expect(result).toBe(8);
    });
    ...
    it("can include Saturday in businessDays given second date falls on a non-businessDay", function () {
      const result = differenceInBusinessDays(newZDT("2022-01-17T00:00:00.000Z"), newZDT("2022-01-09T00:00:00.000Z"), {
        businessDays: [1, 2, 3, 4, 5, 6],
      });
    ...
    it("throws RangeError if businessDays contains numbers greater than 7", function () {
      const block = () =>
        differenceInBusinessDays(newZDT("2022-01-07T00:00:00.000Z"), newZDT("2022-01-14T00:00:00.000Z"), {
          businessDays: [4, 5, 6, 7, 8] as any,
        });
  ...
  describe("exceptions option", function () {
    ...
    it("can handle exceptions that fall on both of the argument dates", function () {
      ...
      const result = differenceInBusinessDays(
        // Sunday
        newZDT("2022-01-16T00:00:00.000Z"),
        // Saturday
    ...
    it("can handle an exception that falls on the second date", function () {
      ...
      const result = differenceInBusinessDays(sunday, saturday, { exceptions });
      expect(result).toBe(6);
    });
    ...
    it("can handle a large amount of enabled Saturday exceptions", function () {
      // with businessDays as Mon-Fri
      const result = differenceInBusinessDays(newZDT("2022-02-14T00:00:00.000Z"), newZDT("2022-01-03T00:00:00.000Z"), {
        // the first and last date are not within the date range, so they should not be added
        exceptions: {
          "2022-01-08": true,
          "2022-01-15": true,
    ...
    it("can handle false exceptions when calculating a 0 day difference", function () {
      const result = differenceInBusinessDays(newZDT("2018-01-01T00:00:00.000Z"), newZDT("2018-01-01T00:00:00.000Z"), {
        businessDays: [7, 1, 2, 3, 4, 5, 6],
    ...
    it("can handle false exceptions when calculating a negative difference", function () {
      const result = differenceInBusinessDays(newZDT("2018-01-01T00:00:00.000Z"), newZDT("2018-01-08T00:00:00.000Z"), {
        businessDays: [7, 1, 2, 3, 4, 5, 6],
    ...
    it("can handle false exceptions when calculating a negative difference across years", function () {
      const result = differenceInBusinessDays(newZDT("2017-12-25T00:00:00.000Z"), newZDT("2018-01-01T00:00:00.000Z"), {
        businessDays: [7, 1, 2, 3, 4, 5, 6],
  ...
  describe("edge cases", function () {
    it("the difference is less than a day, but the given dates are in different calendar days", function () {
      const result = differenceInBusinessDays(newZDT("2014-09-05T00:00:00.000Z"), newZDT("2014-09-04T23:59:00.000Z"));
      expect(result).toBe(1);
    });

    it("the same for the swapped dates", () => {
      const result = differenceInBusinessDays(newZDT("2014-09-04T23:59:00.000Z"), newZDT("2014-09-05T00:00:00.000Z"));
      expect(result).toBe(-1);
    });

    it("the time values of the given dates are the same", () => {
      const result = differenceInBusinessDays(newZDT("2014-09-05T00:00:00.000Z"), newZDT("2014-09-04T00:00:00.000Z"));
      expect(result).toBe(1);
    });

    it("the given dates are the same", () => {
      const result = differenceInBusinessDays(newZDT("2014-09-05T00:00:00.000Z"), newZDT("2014-09-05T00:00:00.000Z"));
      expect(result).toBe(0);
    });
  });
});
