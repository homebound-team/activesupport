 ...
 * @example
 * keyBy([{name: "Alice", age: 25}], p => p.name)
 * //=> {Alice: {name: "Alice", age: 25}}
 * @example
 * keyBy([{name: "Alice", age: 25}], p => p.name, p => p.age)
 * //=> {Alice: 25}
 * @example
 * keyBy([{name: "a"}, {name: "a"}], p => p.name)
 * // throws "a already had a value assigned"
 */
